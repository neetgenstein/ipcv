...
# a. Load the dataset
dataset_dir = pathlib.Path("Fruits Classification")
train_dir = dataset_dir / "train"
val_dir = dataset_dir / "valid"
test_dir = dataset_dir / "test"

# Count images in train directory as a proxy for total images or count all
image_count = len(list(dataset_dir.glob('*/*/*.jpeg'))) + len(list(dataset_dir.glob('*/*/*.png'))) + len(list(dataset_dir.glob('*/*/*.jpg')))
...
# Note: Using existing train and valid directories
...
print("Total images in test_ds:", count)
...
test_loss, test_acc = model_no_aug.evaluate(test_ds, verbose=1)
...
test_loss_aug, test_acc_aug = model_aug.evaluate(test_ds, verbose=1)
print(f"Model 2 Test Accuracy: {test_acc_aug:.4f}")
...
print(f"{'Test Accuracy':<25} | {test_acc:.4f}          | {test_acc_aug:.4f}")
